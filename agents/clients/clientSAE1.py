...
class ClientSAE1:
    ...
    def calculate_loss(self, input):
        ...
        zero_tensor = torch.zeros_like(encode)
        latent_loss = self.loss_function(encode, zero_tensor)
    ...
    def validate(self, epoch):
        self.net.eval()
        with torch.no_grad():
            list_re, list_latent_z, list_loss = [], [], []
            ...
            threshold_re = (np.mean(list_re), np.std(list_re))
            threshold_z = (np.mean(list_latent_z), np.std(list_latent_z))
    ...
    def test(self, is_check=False):
        ...
        for multiplier in np.arange(0.0, 5.1, 0.2):
            ...
            # print(f"[Client {self.client_idx}] Testing with multiplier {multiplier:.1f} - threshold_re: {threshold_re:.4f}, threshold_z: {threshold_z:.4f}")
            is_verbose = (
                not is_check and abs(multiplier - self.args.threshold_multiplier) < 1e-4
            )

            acc, precision, recall, f1, roc = self.test_with_thresholds(
                threshold_re, threshold_z, is_verbose
            )
            acc_list.append(acc)
    ...
    def test_with_thresholds(self, threshold_re, threshold_z, verbose=False):
        self.net.eval()
        with torch.no_grad():
            list_re = []
            list_latent_z = []
            ...
            predictions = [
                int(2 < (r / threshold_re + z / threshold_z))
                for r, z in zip(list_re, list_latent_z)
            ]
