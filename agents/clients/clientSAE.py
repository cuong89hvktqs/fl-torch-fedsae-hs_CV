...
class ClientSAE:
    ...
    def train(self, epoch, pdf_writer_z):
        ...
        for input, _ in self.train_data_loader:
            ...
            re, latent_z = self.calculate_loss(input)
            loss = re + self.args.coef_shrink_ae * latent_z
            loss.backward()
            self.optimizer.step()
        return re, latent_z
    ...
    def validate(self, epoch):
        self.net.eval()
        with torch.no_grad():
            list_re, list_z, list_loss = [], [], []
            for input, _ in self.val_data_loader:
                input = input.to(self.device)
                re, z = self.calculate_loss(input)
                loss = re + self.args.coef_shrink_ae * z
                list_re.append(re.cpu().numpy().tolist())
                list_z.append(z.cpu().numpy().tolist())
                list_loss.append(loss.cpu().numpy().tolist())

            avg_loss = np.mean(list_loss)
            threshold_re = (0, 0)
            threshold_z = (np.mean(list_z), np.std(list_z)) if list_z else 0
    ...
    def test(self, is_check=False):
        ...
        for multiplier in np.arange(0.0, 5.1, 0.2):
            threshold_z = mean_z + multiplier * std_z

            # print(f"[Client {self.client_idx}] Testing with multiplier {multiplier:.1f} -  threshold_z: {threshold_z:.4f}")
            is_verbose = (
                not is_check and abs(multiplier - self.args.threshold_multiplier) < 1e-4
            )

            acc, precision, recall, f1, roc = self.test_with_thresholds(
                threshold_z, is_verbose
            )
            acc_list.append(acc)
    ...
    def test_with_thresholds(self, threshold_z, verbose=False):
        self.net.eval()
        with torch.no_grad():
            list_latent_z = []
            bin_labels = []
            ...
            for input, label in self.test_data_loader:
                ...
                re, latent_z = self.calculate_loss(input)
                list_latent_z.append(latent_z.cpu().numpy().tolist())
                labels = label.cpu().tolist()
                y_bin = (label != 0).int()
                bin_labels.extend(y_bin.cpu().tolist())

            predictions = [1 - int(z <= threshold_z) for z in list_latent_z]

            acc = accuracy_score(bin_labels, predictions)
            precision = precision_score(bin_labels, predictions)
            recall = recall_score(bin_labels, predictions)
            f1 = f1_score(bin_labels, predictions)
            if len(np.unique(bin_labels)) < 2:
                roc =None
            else:
                roc = roc_auc_score(bin_labels, predictions)
            

            # Tính confusion matrix và FPR
            confusion_mat = confusion_matrix(bin_labels, predictions)
            ...
            if verbose:
                ...
                self.args.logger.debug(
                    ...
                    + classification_report(bin_labels, predictions)
                )
    ...
    def test_by_attack_type_full(self, threshold_re, threshold_z, verbose=False):
        ...
        with torch.no_grad():
            ...
            for input, label in self.test_data_loader:
                ...
                list_latent_z.append(
                    latent_z.cpu().item()
                )  # SAE dùng latent_z để phân loại

                binary_label = int(label.item() != 0)
                bin_labels.append(binary_label)
                raw_labels.append(int(label.item()))

            # Dự đoán anomaly theo threshold_z
            predictions = [1 if z > threshold_z else 0 for z in list_latent_z]
