...
class ClientAE:
    ...
    def set_recent_metric(
        self, recent_re, recent_latent_z, recent_train_loss, recent_val_loss, recent_threshold_re, recent_threshold_z
    ):
        """
    ...
    def test_with_thresholds(self, threshold_re, verbose=False):
        ...
        with torch.no_grad():
            ...
            for input, label in self.test_data_loader:
                ...
                labels += (label.cpu().numpy() >= 1).astype(int).tolist()
    ...
    def test_by_attack_type_full(self, threshold, threshold_z, verbose=False):
        self.net.eval()
        with torch.no_grad():
            list_re = []
            bin_labels = []    # binary: 0 (normal), 1 (attack)
            raw_labels = []    # gốc: 0 (normal), 1,2,3... (attack types)
            ...
            # Dự đoán dựa vào threshold
            predictions = [1 if re > threshold else 0 for re in list_re]

            # Gom theo loại tấn công
            results_by_type = defaultdict(lambda: {"y_true": [], "y_pred": []})
            for y, pred, raw_label in zip(bin_labels, predictions, raw_labels):
                results_by_type[raw_label]["y_true"].append(y)
                results_by_type[raw_label]["y_pred"].append(pred)
    ...
    def visualize_z(self, epoch, pdf_writer):
        ...
        if len(latent_z_array) > 200:
            random_indices = np.random.choice(len(latent_z_array), size=200, replace=False)
        ...
        plt.scatter(
            ...
            c="blue", label=f"Client {self.client_idx}"
        )
    ...
    def visualize_validate(self, epoch, threshold_re, threshold_z, auc, list_re, list_latent_z, labels):
        ...
        plt.title(f"Client {self.client_idx} | Epoch {epoch} | AUC: {auc:.6f}\nThresh_Re: {threshold_re:.6f} | Thresh_Z: {threshold_z:.6f}")
        plt.xlabel("Latent Z")
        plt.ylabel("Reconstruction Error")

        # **Vẽ điểm dữ liệu**
        plt.scatter(list_latent_z[labels == 0], list_re[labels == 0], c='blue', s=12, label="Normal (0)", alpha=0.4)
        plt.scatter(list_latent_z[labels == 1], list_re[labels == 1], c='red', s=12, label="Anomalous (1)", alpha=0.4)
