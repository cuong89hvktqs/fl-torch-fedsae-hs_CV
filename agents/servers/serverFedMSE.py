...
from agents.clients.clientFedMSE import ClientFedMSE
...
class ServerFedMSE:
    def __init__(self, args):
        ...
        if args.aggregation_type == "FedMSE":
            self.client_for_eval = ClientFedMSE(
                args,
                0,
                None,
                None,
    ...
    def train_on_clients(self, epoch, clients, poisoned_workers, pdf_writer):
        ...
        for client_idx in random_workers:
            ...
            if client.is_training:
                ...
                val_loss, threshold_re, threshold_z = client.validate()
        ...
        if len(list_client_training) > 0:
            ...
            elif self.args.aggregation_type == "FedMSE":
                new_nn_params = self.fed_mse_avg(parameters)
                for client_idx in list_client_training:
    ...
    def fed_mse_avg(self, local_models):
        """
        Perform federated model aggregation using similarity-based weights (inverse of MSE).
        Args:
            local_models: list of state_dict from each client

        Returns:
            avg_weights: dictionary of averaged model weights
        """
        update_weights = []
        for state_dict in local_models:
            # Load client weights
            self.client_for_eval.update_nn_parameters(state_dict)

            # Tính MSE giữa input và output
            sim_score = self.client_for_eval.cal_loss_mse(self.args.dev_dataset)
            weight = 1.0 / (sim_score.item() + 1e-8)  # tránh chia 0

            update_weights.append((state_dict, weight))

        # Tính trung bình trọng số theo 1/MSE
        avg_weights = {}
