...
class ServerAE:
    ...
    def train_on_clients(self, epoch, clients, poisoned_workers, pdf_writer):
        ...
        for client_idx in random_workers:
            ...
            if client.is_training:
                ...
                client.set_recent_metric(train_re, None, train_loss, val_loss, threshold_re, threshold_z)
                ...
                # 💾 Lưu mô hình client mỗi 10 epoch
        ...
        if len(list_client_training) > 0:
            ...
            if self.args.aggregation_type == "cluster" and len(list_client_training) > 1:
                ...
                # Các loại aggregation khác
                new_nn_params = self.aggregate_parameters(parameters, list_loss)#tổng hợp mô hình của các client đâng huấn luyện
                for client_idx in list_client_training:
    ...
    def aggregate_parameters(self, parameters, list_loss):
        if self.args.aggregation_type == "average":
            return average_nn_parameters(parameters) #tổng hợp bằng trung bình chung
